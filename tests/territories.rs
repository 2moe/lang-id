...
//! license.
//! - <https://github.com/unicode-org/cldr-json/blob/main/cldr-json/cldr-localenames-full/main/en-001/territories.json>
//!
...
use compact_str::CompactString as StrBuf;
...
#[test]
#[ignore]
fn convert_json_data() -> AnyResult<()> {
  let map = en_001_territories_json()
    .pipe(serde_json::from_str::<LocaleDisplayNames>)?
    .territories;
...
const fn en_001_territories_json() -> &'static str {
  r##"{
      "territories": {
          "001": "world",
          "002": "Africa",
...
struct LocaleDisplayNames {
  territories: HashMap<StrBuf, StrBuf>,
}

fn build_ids_map(map: &HashMap<StrBuf, StrBuf>) -> io::Result<()> {
  let vec = map
    .iter()
...
fn build_names_map(map: &HashMap<StrBuf, StrBuf>) -> io::Result<()> {
  let vec = map
    .iter()
    .filter(|(k, _)| k.as_str() != "IO-alt-biot")
